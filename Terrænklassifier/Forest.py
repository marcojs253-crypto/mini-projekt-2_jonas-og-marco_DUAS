    ...
    for i in range(1, 75):
        ...
        for y in range(5):
            for x in range(5):
                ...
                forest_pixels = cv2.countNonZero(tile_mask)
                
                # Beregn procentdel i stedet for absolut antal
                tile_total_pixels = tile.shape[0] * tile.shape[1]
                forest_percentage = (forest_pixels / tile_total_pixels) * 100
                
                print(f"Tile ({y},{x}) forest pixels: {forest_pixels} ({forest_percentage:.1f}%)")
                
                # Filtrer hvis > 30% af tile er forest
                if forest_percentage > 30:  
                    # Dette ER et forest-felt, gem det
                    output_filename = f"{output_folder}/forest_{i}_{y}_{x}.jpg"
                    cv2.imwrite(output_filename, tile)
                    print(f"  -> Gemt som forest_{i}_{y}_{x}.jpg")
                else:
                    tiles[y][x][:] = 0  # Ikke et forest, gør sort
